...
The true kernel value there is below $10^{-323}$, so the loss is numerically
negligible. Asserted in `test_rbf_kernel_underflows_to_zero_at_distance`.
...
$$
\frac{\partial k_\ell(x,z)}{\partial \ell}
=
\exp\left(-\frac{r^2}{2\ell^2}\right)
\frac{r^2}{\ell^3}.
$$
...
$$
\exp\left(-\frac{r^2}{2\ell^2}\right)>0,
\qquad
r^2>0,
\qquad
\ell^3>0,
...
$$
\boxed{
\frac{\partial k_\ell(x,z)}{\partial\ell}>0
}
\qquad (x\neq z).
$$
