...
def rbf_kernel(X, Z=None, lengthscale=1.0):
    ...
    if X.ndim != 2:
        raise ValueError(f"X must be 2-D of shape (n_samples, n_features), got shape {X.shape}")
    ...
    if Z is None:
        ...
        if Z.ndim != 2:
            raise ValueError(f"Z must be 2-D of shape (n_samples, n_features), got shape {Z.shape}")
